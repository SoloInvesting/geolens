...
המפה היא משטח העבודה הראשי של GeoLens. אפשר לעבור בין תצלום לוויין למפת רחובות, להדליק ולכבות בנפרד את אזור החיפוש, טביעות הסצנות, תצוגת המקור, אירועי הקטלוג ותוצאת המודל, ולפתוח כל שכבה כדי לראות את הראיה והמקור שלה.

אפשר לסמן פוליגון AOI ישירות על המפה. כאשר שולחים את הבקשה, הפוליגון הופך לאזור המשימה הקנוני, והסוכן מחפש ומנתח אותו בלי לבקש שוב עיר או מדינה. עבור שתי סצנות עם תצוגת Quicklook זמינה, תצוגת תצלום הלוויין כוללת מחוון השוואה. זו השוואה חזותית של תמונות המקור, לא תחליף למסכת מודל או למדידה גאודזית.

חלונית המקורות מציגה קישור STAC לכל סצנה וקישורים לנכסים הסמנטיים הזמינים, כגון RGB, NIR או COG. נכס שמסומן כ-Requester Pays או כדרש הרשאה מוצג כמוגבל ולא נשלח למודל כאילו היה ציבורי. כפתור `מצב מפה` מייצא `MapSession` מקומי עם שכבות, גאומטריות וקישורי ראיות, ללא סודות.
